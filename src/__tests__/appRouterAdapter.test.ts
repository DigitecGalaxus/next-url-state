...
type MockRouter = Pick<AppRouterInstance, 'push' | 'replace'>;
...
describe('createAppRouterAdapter', () => {
  ...
  it('should return type "app"', () => {
    const adapter = createAppRouterAdapter('/test', new URLSearchParams(), mockRouter as AppRouterInstance);
  ...
  describe('getCurrentPath', () => {
    it('should return pathname only when no search params and no hash', () => {
      window.location.hash = '';
      const adapter = createAppRouterAdapter('/products', new URLSearchParams(), mockRouter as AppRouterInstance);
    ...
    it('should return pathname with search params', () => {
      ...
      const adapter = createAppRouterAdapter('/search', searchParams, mockRouter as AppRouterInstance);
      expect(adapter.getCurrentPath()).toBe('/search?q=test&page=1');
    });

    it('should include hash from window.location', () => {
      window.location.hash = '#top';
      const adapter = createAppRouterAdapter('/page', new URLSearchParams(), mockRouter as AppRouterInstance);
    ...
    it('should return pathname with search params and hash', () => {
      ...
      const adapter = createAppRouterAdapter('/article', searchParams, mockRouter as AppRouterInstance);
  ...
  describe('updateUrl', () => {
    it('should call router.push for push method', async () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/test?foo=bar', { scroll: false });
    ...
    it('should call router.replace for replace method', async () => {
      ...
      expect(mockRouter.replace).toHaveBeenCalledWith('/test?foo=bar', { scroll: false });
    ...
    it('should include hash in the URL', async () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/test?foo=bar#section', { scroll: false });
    });
    ...
    it('should handle empty params', async () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/page', { scroll: false });
    });
    ...
    it('should handle multiple params', async () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/multi?a=1&b=2&c=3', { scroll: false });
    });
    ...
    it('should handle array params', async () => {
      ...
      expect(mockRouter.push).toHaveBeenCalledWith('/posts?tags=react&tags=nextjs', { scroll: false });
    });
